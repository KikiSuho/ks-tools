"""
macOS-specific platform data and behavior for scrutiny.
...
Examples
--------
>>> from scrutiny.platforms.macos import get_pathext
>>> get_pathext()
['']
...
IDE_ENV_VARS: frozenset[str] = frozenset(
    {
        ...
        "SUBLIME_TEXT",
        "ATOM_HOME",
        # NOTE: TERM_PROGRAM_VERSION is intentionally excluded;
        # macOS Terminal.app and iTerm2 both set it, causing false
        # positive IDE detection.
    },
)
...
def get_pathext() -> list[str]:
    """Return executable extensions for macOS (no extensions needed)."""
    return [""]
...
def get_extra_search_dirs(_interpreter_dir: str) -> list[str]:
    ...
    On macOS, no extra directories are needed; ``bin/`` is already
    the standard location and is typically on PATH.

    Parameters
    ----------
    _interpreter_dir : str
        Directory containing the running Python interpreter (unused on macOS).

    Returns
    -------
    list[str]
        Empty list (no extra dirs on macOS).
...
def get_subprocess_creation_flags() -> int:
    """Return creation flags for subprocess calls on macOS."""
    return _NO_CREATION_FLAGS


def get_subprocess_preexec_fn() -> Optional[Callable[[], None]]:
    """
    Return pre-exec function for subprocess calls on macOS.
...
def safe_rmtree(directory: Path) -> None:
    ...
    On macOS, no special handling is needed; ``shutil.rmtree``
    works directly.
...
def terminate_process_tree(pid: int) -> None:
    """
    Terminate a process and all its children on macOS.
    ...
    # Attempt group-level termination first, fall back to single process.
    try:
        # Send SIGTERM to the entire process group.
        os.killpg(os.getpgid(pid), signal.SIGTERM)
    except (ProcessLookupError, PermissionError):
        # Group lookup failed; try killing the single process instead.
